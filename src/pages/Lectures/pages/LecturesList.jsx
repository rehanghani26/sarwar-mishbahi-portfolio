    ...
    <div className="bg-[#FAF9F5] dark:bg-slate-900 py-12 min-h-screen text-right">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 text-center">
          <span className="text-xs font-bold text-[#8A6F52] dark:text-amber-500 uppercase tracking-widest font-serif block mb-1">ملٹی میڈیا لائبریری</span>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-[#2F241C] dark:text-[#8A6F52] font-serif tracking-wide">خطابات اور بیانات</h1>
        ...
        <div className="premium-card p-5 mb-10 flex flex-col md:flex-row items-center justify-between gap-5 text-right">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto shrink-0 justify-start md:justify-end">
            ...
            >
              <option value="">تمام فارمیٹس</option>
              {categories.map((cat) => (
        ...
        {loading ? (
          ...
          <div className="text-center py-16 premium-card">
            ...
            <h3 className="text-lg font-bold text-slate-700 dark:text-white font-serif">کوئی بیان نہیں ملا</h3>
            <p className="text-slate-550 dark:text-slate-400 text-xs mt-1">براہ کرم فارمیٹ فلٹر بیجز یا تلاش کے الفاظ تبدیل کریں۔</p>
          </div>
        )}
      ...
      {activeMedia && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/80 backdrop-blur-xs p-4">
          <div className="premium-card rounded-lg shadow-2xl overflow-hidden w-full max-w-3xl relative flex flex-col text-right">

            {/* Modal Header */}
            <div className="bg-[#2F241C] dark:bg-slate-900 text-white px-5 py-3.5 flex items-center justify-between border-b border-[#8A6F52]/35 dark:border-slate-700 text-right">
              <h3 className="font-bold text-sm sm:text-md font-serif line-clamp-1 pl-6 text-right">{activeMedia.title}</h3>
              <button
              ...
              {isAudioMedia(activeMedia.category) ? (
                <div className="w-full h-full flex flex-col items-center justify-center bg-slate-950 p-6 text-center gap-6">
                  ...
                  <div className="space-y-1">
                    <span className="text-xs text-slate-400 font-bold uppercase tracking-wider block">آڈیو بیان چل رہا ہے</span>
                ...
                <div className="w-full h-full flex flex-col items-center justify-center p-6 text-center bg-slate-900 text-white gap-4 text-right">
                  <Play className="w-12 h-12 text-[#8A6F52]" />
                  <p className="text-sm text-slate-300 max-w-sm font-light text-right">
            ...
            <div className="p-5 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-700 max-h-32 overflow-y-auto text-right">
              <span className="block text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase mb-1 text-right">تفصیل</span>
              <p className="text-slate-700 dark:text-slate-300 text-xs font-light leading-relaxed text-right">{activeMedia.description}</p>
            </div>
