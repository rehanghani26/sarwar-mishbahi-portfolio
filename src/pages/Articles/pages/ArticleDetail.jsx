  ...
  if (error || !current) {
    return (
      <div className="max-w-xl mx-auto px-4 py-20 text-center" dir={language === 'ur' ? 'rtl' : 'ltr'}>
        ...
        <Link to="/articles" className="inline-flex items-center gap-1.5 mt-6 px-4 py-2 bg-primary text-white rounded font-semibold text-sm hover:bg-primary/90">
  ...
  return (
    <div className={`bg-background py-12 min-h-screen ${language === 'ur' ? 'text-right' : 'text-left'}`} dir={language === 'ur' ? 'rtl' : 'ltr'}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        <Link to="/articles" className="inline-flex items-center gap-1 text-sm font-bold text-textPrimary hover:text-accent dark:hover:text-amber-400 mb-6">
        ...
        <article className="premium-card overflow-hidden mb-12">
          ...
          <div className="h-64 sm:h-[400px] w-full bg-slate-100 dark:bg-slate-950 relative">
            ...
            />
            <div className={`absolute top-4 ${language === 'ur' ? 'left-4' : 'right-4'} bg-primary dark:bg-primary text-white text-xs font-bold uppercase tracking-wider px-3.5 py-1.5 rounded shadow-md`}>
              {category}
            </div>
          </div>
          ...
          <div className="p-6 sm:p-10">
            ...
            {/* Title & Summary */}
            <h1 className={`text-2xl sm:text-3xl lg:text-4xl font-extrabold text-textPrimary font-serif leading-tight mb-4 tracking-wide ${language === 'ur' ? 'text-right' : 'text-left'}`}>
              {title}
            </h1>
            <p className={`text-slate-700 dark:text-slate-300 text-md font-light leading-relaxed italic ${
              language === 'ur' ? 'border-r-4 pr-4 text-right border-accent dark:border-amber-500' : 'border-l-4 pl-4 text-left border-accent dark:border-amber-500'
            } mb-8`}>
              {summary}
            </p>
            ...
            <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs text-slate-500 dark:text-slate-400 font-medium flex items-center gap-1 shrink-0">
                  <Share2 className="w-4 h-4 text-accent dark:text-amber-500" /> 
                  {language === 'en' ? 'Share:' : 'شیئر کریں:'}
                </span>
                <button
                  onClick={() => handleShareClick('facebook')}
                  className="p-2 rounded bg-slate-100 dark:bg-slate-800 text-textPrimary hover:bg-primary dark:hover:bg-primary hover:text-white dark:hover:text-white transition-colors"
        ...
        {related && related.length > 0 && (
          <div>
            <h3 className={`text-xl font-bold text-textPrimary mb-6 pb-2 border-b border-border ${language === 'ur' ? 'text-right' : 'text-left'}`}>
