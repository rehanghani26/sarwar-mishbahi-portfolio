...
export default function EventCard({ event }) {
  ...
  return (
    <div className="premium-card shadow-sm overflow-hidden flex flex-col h-full md:flex-row group text-right">
      ...
      <div className="relative h-48 md:h-auto md:w-48 bg-slate-100 dark:bg-slate-900 shrink-0 overflow-hidden">
        ...
        <div className={`absolute top-3 right-3 text-xs font-bold uppercase tracking-wider px-2.5 py-1 rounded-full shadow-sm ${
          ...
          {isUpcoming ? 'آنے والا' : 'گزشتہ پروگرام'}
        </div>
      </div>

      {/* Card Content */}
      <div className="p-4 flex flex-col flex-grow md:flex-row items-stretch gap-4 text-right">

        {/* Date Stamp Block (Right Side on Desktop in RTL) */}
        <div className="flex flex-row md:flex-col items-center justify-center border-b md:border-b-0 md:border-l border-[#E5D8CA] pb-4 md:pb-0 md:pl-4 shrink-0 text-center gap-4">
        ...
        {/* Textual Details */}
        <div className="flex flex-col flex-grow justify-between text-right">
          <div>
            {/* Title */}
            <h3 className="text-base font-bold text-slate-900 group-hover:text-[#1F3A5F] transition-colors leading-snug mb-4 font-serif text-right">
              {title}
            </h3>

            {/* Description */}
            <p className="text-[#2C2C2C] text-sm font-light leading-relaxed line-clamp-3 mb-4 text-right">
              {description}
            </p>
          </div>

          {/* Time and Location markers */}
          <div className="flex flex-col gap-4 pt-4 border-t border-slate-100 text-sm text-slate-500 justify-start text-right">
            ...
            <span className="flex items-start gap-2 text-right">
              ...
              <span className="line-clamp-1 text-right">{location}</span>
            </span>
          </div>
        </div>
