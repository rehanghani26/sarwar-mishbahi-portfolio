...
function ArticlesCol() {
  return (
    <div className="bg-card-bg border-r border-site-border">
      <ColHeader title="مقالات" />
      {ARTICLES.map((a) => (
        <div key={a.id} className="flex items-start gap-3 p-[15px] px-[18px] border-b border-site-border last:border-b-0 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
          <div className="flex-1 text-right">
            <div className="text-[16px] text-text-primary leading-[1.75] mb-1">{a.title}</div>
            <a href="#" className="inline-flex items-center gap-1 text-[14px] text-brown-mid dark:text-amber-500 font-semibold hover:text-brown-dark dark:hover:text-white">
...
function PrayerCol() {
  ...
  return (
    <div className="bg-card-bg border-r border-site-border">
      ...
      <div className="bg-cream-light dark:bg-slate-800 py-[11px] px-4 text-center text-[14px] text-text-primary font-semibold border-b border-site-border">
        {PRAYER_TIMES.date}
      </div>
      <table className="w-full border-collapse">
        <tbody>
          {rows.map((row, ri) => (
            <tr key={ri} className="border-b border-site-border last:border-b-0">
              {row.map((p, ci) => (
...
function BooksCol() {
  return (
    <div className="bg-card-bg">
      ...
      {BOOKS.map((b) => (
        <div key={b.id} className="flex items-start gap-3 p-[15px] px-[18px] border-b border-site-border last:border-b-0 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
          <div className="flex-1 text-right">
            <div className="text-[15px] text-text-primary leading-[1.75] mb-1">{b.title}</div>
            <a href="#" className="inline-flex items-center gap-1 text-[14px] text-brown-mid dark:text-amber-500 font-semibold hover:text-brown-dark dark:hover:text-white">
              <span>›</span> پڑھنے کے لیے کلک کریں
            </a>
          </div>
...
export default function ThreeColumns() {
  return (
    <section className="px-4 pb-4 md:px-7 md:pb-7" aria-label="مقالات، نماز کے اوقات، کتابیں">
      <div className="max-w-[1440px] mx-auto grid grid-cols-1 lg:grid-cols-3 border border-site-border border-t-0">
        <ArticlesCol />
        <PrayerCol />
        <BooksCol />
      </div>
