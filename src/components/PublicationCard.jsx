...
export default function PublicationCard({ publication }) {
  ...
  return (
    <div className="premium-card p-5 flex flex-col h-full hover:shadow-md transition-all group text-start">
      ...
      {/* Author Details */}
      <span className="block text-xs font-semibold text-[#B08D57] mb-3 shrink-0">
        {t('By')} {author}
      </span>

      {/* Description */}
      <div className="flex-grow">
        <p className="text-[#2C2C2C] text-xs font-light leading-relaxed line-clamp-3">
          {description}
        </p>
      </div>

      {/* Drive Action link buttons */}
      <div className="mt-5 pt-3 border-t border-slate-100 flex items-center justify-between gap-3 shrink-0">
        <a
